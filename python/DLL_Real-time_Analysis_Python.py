...
from matplotlib.lines import Line2D  # stable legend proxies
from matplotlib import dates as mdates  # <-- for UTC time x-axis
from matplotlib.ticker import FuncFormatter  # <-- custom ms formatter
from brainflow.board_shim import BoardShim, BrainFlowInputParams, BoardIds
from brainflow.data_filter import DataFilter, FilterTypes  # optional bandpass
import mindsai_filter_python

# Minds AI Filter Real-time testing project. Created by JM Wesierski
# Next Steps:
# Add noise highlights to real EEG noises
...
# === CONFIG ===
filterHyperparameter = 1e-6
channel_idx = 6            # EEG channel to display 0-based (0–15)
window_seconds = 1
...
BP_HIGH_HZ      = 40.0
BP_ORDER        = 4
BP_TYPE         = FilterTypes.BUTTERWORTH_ZERO_PHASE  # zero-phase recommended
...
SNR_METHOD = "variance_ratio"

# --- Board Params ---
USE_BRAINFLOW = True 
USE_SYNTHETIC = False        # Set to True for testing without real device
...
# --- Metrics: SNR and Filter Impact (Reusable) ---
...
# --- Pretty, contextual console summary with inline tags (thresholded) ---
...
# --- Optional: apply bandpass to data that will feed MindsAI ---
...
try:
    while True:
        ...
        if (all(len(buf) == window_size for buf in buffers)) and (len(time_buffer) == window_size):
            ...
            filtered = mindsai_filter_python.mindsai_python_filter(prefiltered_for_mindsai, filterHyperparameter)
            if filtered.shape != raw_window.shape:
                filtered = filtered.T if filtered.T.shape == raw_window.shape else ValueError("Shape mismatch")

            raw_ch  = raw_window[channel_idx]
            filt_ch = filtered[channel_idx]
            ...
            legend_handles = [
                Line2D([], [], color="#ffffff", linestyle="-",      label="Cyton Daisy Raw"),
            ...
            avg_raw  = np.mean(raw_window, axis=0)
            avg_filt = np.mean(filtered, axis=0)
